  ...
  return (
    <div className="p-4 md:p-10 max-w-7xl mx-auto">
      {stats!.canli > 0 && (
        <div className="mb-4 md:mb-6 bg-ember-50 border border-ember-200 rounded-2xl px-4 py-3 md:px-6 md:py-4 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 md:gap-3 min-w-0">
            ...
            <span className="text-ember-700 font-medium text-sm truncate">{stats!.canli} müşteri bekliyor</span>
          </div>
          <a href="/canli-destek" className="shrink-0 px-3 py-1.5 bg-ember-600 text-white text-xs font-medium rounded-xl hover:bg-ember-700 transition-colors">
            Aç →
          </a>
        </div>
      )}
      ...
      <header className="mb-6 md:mb-12">
        <p className="text-xs uppercase tracking-[0.3em] text-ink-300 font-medium mb-2 md:mb-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 md:gap-4 mb-6 md:mb-12">
      ...
      <div className="bg-white border border-cream-200 rounded-2xl overflow-hidden">
        <div className="px-4 md:px-8 py-4 md:py-6 border-b border-cream-200 flex items-center justify-between">
          <h2 className="font-display text-xl md:text-2xl text-ink-900">Son Konuşmalar</h2>
            ...
            <thead>
              <tr className="bg-cream-50">
                {['Müşteri','Son Mesaj','Niyet','Durum','Zaman'].map(h => (
                  <th key={h} className="px-4 md:px-8 py-3 text-left text-[10px] uppercase tracking-[0.2em] text-ink-300">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {sessions.map(s => (
                <tr key={s.phone} className="border-t border-cream-100 hover:bg-cream-50 cursor-pointer" onClick={() => router.push('/konusmalar')}>
                  <td className="px-4 md:px-8 py-3 font-mono text-sm text-ink-700">{s.phone}</td>
                  <td className="px-4 md:px-8 py-3 text-sm text-ink-500 max-w-[150px] truncate">{s.musteri_yazdigi || '—'}</td>
                  ...
                  <td className="px-4 md:px-8 py-3 text-xs text-ink-300 font-mono whitespace-nowrap">
                    {formatDistanceToNow(new Date(s.updated_at), { addSuffix: true, locale: tr })}
                  </td>
                </tr>
              ))}
              {sessions.length === 0 && <tr><td colSpan={5} className="px-8 py-12 text-center text-ink-300 font-mono text-sm">henüz konuşma yok</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
